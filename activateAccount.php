<?php
	...
	$query = "UPDATE registrationTable SET status = ?, activationCode = ? WHERE userName = ?"; //Update query
	$stmt = $conn->prepare($query); //Processing prepared statement
	$stmt->bind_param("iss", $status, $activationCode, $userName); //Binding parameters
